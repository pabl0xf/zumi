...
class Crop:
    ...
    def crop(self, image):
        ...
        self.threshhold_image = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY)[1]
        ...
        if height > width + 2:
                # print("a")
            img_cropped = image[self.crop_pixel.up:self.crop_pixel.up + width, self.crop_pixel.left:self.crop_pixel.right]
        else:
            img_cropped = image[self.crop_pixel.up:self.crop_pixel.down, self.crop_pixel.left:self.crop_pixel.right]
