...
sequencesInteface.square = async function() {
  ...
  var promiseCommand = new Promise(async function(resolve, reject) {
    await global.move(2, 50, 0, 0, 0);
    await global.move(2, 0, 50, 0, 0);
    await global.move(2, -50, 0, 0, 0);
    await global.move(2, 0, -50, 0, 0);
...
sequencesInteface.triangle = async function() {
  ...
  var promiseCommand = new Promise(async function(resolve, reject) {
    await move(1.5, 0,50,0,0);
    console.log('first left 120');
  	await turnDegree(LEFT,120);
    console.log('first move');
  	await move(1.5, 0,50,0,0);
  	await turnDegree(LEFT,120);
    console.log('second left 120');
  	await move(1.5, 0,50,0,0);
